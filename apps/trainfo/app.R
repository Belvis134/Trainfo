...
ui <- fluidPage(
  ...
  titlePanel(tags$p(style = "color: white; text-align: center", "Traⓘnfo Alpha 1.2.0")),
  sidebarLayout(
    sidebarPanel(
      width = 6,
      ...
      conditionalPanel(
        ...
        textInput("remarks_input", "Any remarks? Describe how the breakdown/accident happened and perpetrators if any. This is optional."),
...
server <- function (input, output, session) {
  ...
  format_data <- function(df) {
    ...
    df$LINE <- sapply(str_split(df$LINE, "\n"), function(x) list(x))
  ...
  observeEvent(input$train_line, {
    ...
    dl <- c(
      ...
      "CCL - Clockwise via Dhoby Ghaut" = "CCL-CW-DBG",
      "CCL - Anti-Clockwise via Dhoby Ghaut" = "CCL-ACW-DBG",
    ...
    if ("CCL" %in% line) {dir_choices <- c(
      dir_choices, dl["CCL - Clockwise"], dl["CCL - Anti-Clockwise"], dl["CCL - Clockwise via Dhoby Ghaut"], dl["CGL - Changi Airport"]
    ...
    updateSelectizeInput(session, "direction", choices = dir_choices)
    updateSelectInput(session, "stations_from", choices = valid_names)
    updateSelectInput(session, "stations_till", choices = valid_names)
  })
  ...
  add_sector <- observeEvent(input$add_sector, {
    d <- prep_df()
  ...
  modify_sector <- observeEvent(input$modify_sector, {
    d <- prep_df()
    if(!d$sector %in% d$existing_df$SECTOR) stop(paste0("There is no sector", d$sector, "to modify!"))
  ...
  delete_sector <- observeEvent(input$delete_sector, {
    existing_df <- sheet_out2()
    sector <- input_sector()
  ...
  submitted_report <- observeEvent(input$submit_report, {
    existing_df <- sheet_out2()
  ...
  observeEvent(input$submit_report, {
    showModal(
      modalDialog(
        ...
        "Oi, I haven't implemented this yet lah!",
        easyClose = TRUE,  # Allow closing by clicking outside
        footer = tagList(
          modalButton("Close")
        )
